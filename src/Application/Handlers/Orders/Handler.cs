...
public class Handler
{
    ...
    public async Task<DTOs.Responses.Orders.Update?> UpdateAsync(DTOs.Requests.Orders.Update request)
    {
        try
        {
            ...
            // Remove existing order lines from database
            var existingLines = await _context.OrderLines.Where(ol => ol.OrderId == order.Id).ToListAsync();
            foreach (var line in existingLines)
            {
                _context.OrderLines.Remove(line);
            }

            // Clear the order's lines collection
            order.Lines.Clear();

            // Add new order lines to database
            foreach (var newLine in newOrderLines)
            {
                await _context.OrderLines.AddAsync(newLine);
            }

            order.SetUpdatedAt();
            await _unitOfWork.CommitAsync();

            // Reload the order with updated lines
